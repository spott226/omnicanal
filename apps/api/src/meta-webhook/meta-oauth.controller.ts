...
import { CompleteWhatsAppSignupDto } from "../resource.dto";
...
<html lang="es">
  ...
  @Get("facebook/start")
  @Protected("SUPER_ADMIN", "ORGANIZATION_ADMIN", "SUPERVISOR")
  startFacebook(@CurrentPrincipal() principal: AuthPrincipal) {
    return this.oauth.startFacebookLogin(principal);
  }
  ...
  async facebookCallback(
    ...
    const result = await this.oauth.completeFacebookLogin({ code, error, errorDescription, state, redirectUri: this.publicFacebookCallbackUrl(request) });
    return this.connectedPage("Facebook", result.username || result.externalAccountId || "Página de Facebook");
  }

  @Post("whatsapp/complete")
  @Protected("SUPER_ADMIN", "ORGANIZATION_ADMIN", "SUPERVISOR")
  completeWhatsApp(@CurrentPrincipal() principal: AuthPrincipal, @Body() dto: CompleteWhatsAppSignupDto) {
    return this.oauth.completeWhatsAppEmbeddedSignup(principal, dto);
  }
  ...
  private publicFacebookCallbackUrl(request?: any) {
    const host = request?.headers?.["x-forwarded-host"] || request?.headers?.host;
    const proto = request?.headers?.["x-forwarded-proto"] || request?.protocol || "https";
    if (!host) return undefined;
    return `${proto}://${host}/api/v1/meta/facebook/callback`;
  }

  private connectedPage(channel: string, account: string) {
    const safeChannel = this.escapeHtml(channel);
    const safeAccount = this.escapeHtml(account);
    return `<!doctype html><html lang="es"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>${safeChannel} conectado</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#07111f;color:#e8f7ff;font-family:Arial,sans-serif}main{width:min(560px,calc(100vw - 32px));border:1px solid rgba(34,211,238,.35);border-radius:22px;padding:32px;background:linear-gradient(135deg,rgba(15,23,42,.96),rgba(8,47,73,.72));box-shadow:0 24px 80px rgba(34,211,238,.16)}p{color:#a9c6d9;line-height:1.6}.ok{color:#2fffd2;font-weight:800;letter-spacing:.14em;text-transform:uppercase;font-size:12px}h1{margin:10px 0 8px;font-size:34px}button{margin-top:18px;border:0;border-radius:14px;padding:14px 18px;color:#02111d;background:linear-gradient(135deg,#38e8ff,#7c4dff);font-weight:800;cursor:pointer}</style></head><body><main><div class="ok">Conexion lista</div><h1>${safeChannel} conectado</h1><p>La cuenta <strong>${safeAccount}</strong> quedó autorizada para next.io by Mercadia. Ya puedes cerrar esta pestaña y volver al panel.</p><button onclick="window.close()">Cerrar pestaña</button></main></body></html>`;
  }
}
