...
import { api, type ApiConversation, type ApiDashboard, type ApiMessage, type BillingInterval, type BillingMockAction, type BillingPlan, type BillingUsage, type KnowledgeKind, type KnowledgeRecord, type PlanPrice, type SubscriptionInfo, type TeamMember, type TeamRole } from "./api-client";
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {view === "Conversaciones" && (active ? <Inbox conversations={conversations} setConversations={setConversations} active={active} setActiveId={setActiveId} notify={notify} onAppointment={() => setModal(true)} /> : <EmptyState title="Sin conversaciones todavía" note="Cuando conectes canales reales o cargues conversaciones, aparecerán aquí. No se muestran datos demo en cuentas nuevas." />)}
...
function Channels({notify}:{notify:(s:string)=>void}){
  ...
  return <section className="page-stack"><div className="section-heading"><div><h2>Canales</h2><p>Administra los puntos de contacto de tu organización.</p></div><span className="demo-banner"><i/> Modo demostración · CHANNEL_PROVIDER_MODE=mock · Meta real no conectado</span></div><div className="channel-cards">{(["instagram","whatsapp","facebook"] as Channel[]).map((channel,i)=><article className="card channel-card" key={channel}><div><i className={`channel huge ${channel}`}>{CHANNELS[channel].short}</i><span><h3>{CHANNELS[channel].label}</h3><p>{i===0?"Direct y comentarios":i===1?"WhatsApp Cloud API":"Messenger y comentarios"}</p></span></div><span className="simulated">● Conectado mock</span><div className="channel-details"><p><span>Cuenta mock</span><b>{i===0?"@mercadia_mock":i===1?"+52 55 0000 2026":"Mercadia Mock Page"}</b></p><p><span>Proveedor</span><b>mock · sin Meta real</b></p></div><dl><div><dt>Conversaciones</dt><dd>{CHANNEL_DATA[channel].count}</dd></div><div><dt>Estado mock</dt><dd className={connected[channel]?"positive":""}>{connected[channel]?"Conectado mock":"No conectado"}</dd></div></dl><button onClick={()=>notify(`Evento mock de ${CHANNELS[channel].label} verificado. No se llamó a Meta.`)}>Probar evento mock</button><button onClick={()=>notify(`${CHANNELS[channel].label}: sin eventos reales; proveedor mock activo`) }>Ver eventos recientes</button><button className="primary" onClick={()=>{setConnected({...connected,[channel]:!connected[channel]});notify(connected[channel]?"Canal mock desconectado":"Canal mock conectado")}}>{connected[channel]?"Desconectar mock":"Conectar mock"}</button></article>)}</div><div className="card meta-ready"><span>◎</span><div><h3>Arquitectura preparada para Meta</h3><p>Esta fase solo centraliza el proveedor mock. No se envían tokens, webhooks ni solicitudes reales a Meta.</p></div><button onClick={()=>notify("Meta real se conecta hasta la fase final de integraciones")}>Ver criterio de conexión →</button></div></section>
}

const teamRoleLabel = (role: TeamRole) => role === "ORGANIZATION_ADMIN" ? "Administrador" : role === "SUPERVISOR" ? "Supervisor" : "Agente";
const initialsFromName = (name: string) => name.split(" ").slice(0,2).map(part=>part[0]).join("").toUpperCase() || "NX";

function TeamPanel({notify}:{notify:(s:string)=>void}) {
  const [members,setMembers]=useState<TeamMember[]>([]);
  const [loading,setLoading]=useState(true);
  const load=async()=>{setLoading(true);try{setMembers(await api.teamMembers())}catch(reason){notify(reason instanceof Error?reason.message:"No se pudo cargar equipo")}finally{setLoading(false)}};
  useEffect(()=>{void load()},[]);
  const invite=async()=>{const name=window.prompt("Nombre del miembro");if(!name?.trim())return;const email=window.prompt("Correo del miembro");if(!email?.trim())return;try{const member=await api.teamInvite({name:name.trim(),email:email.trim(),role:"AGENT"});setMembers(current=>[...current,member]);notify(member.temporaryPassword?`Miembro creado. Contraseña temporal: ${member.temporaryPassword}`:"Miembro agregado")}catch(reason){notify(reason instanceof Error?reason.message:"No se pudo agregar miembro")}};
  const changeRole=async(member:TeamMember,role:TeamRole)=>{try{const updated=await api.teamUpdateRole(member.id,role);setMembers(current=>current.map(item=>item.id===member.id?updated:item));notify("Rol actualizado")}catch(reason){notify(reason instanceof Error?reason.message:"No se pudo actualizar rol")}};
  const remove=async(member:TeamMember)=>{if(!window.confirm(`¿Quitar a ${member.user.name} del equipo?`))return;try{await api.teamRemove(member.id);setMembers(current=>current.filter(item=>item.id!==member.id));notify("Miembro removido")}catch(reason){notify(reason instanceof Error?reason.message:"No se pudo quitar miembro")}};
  return <><CardTitle title="Equipo y roles" note={loading?"Sincronizando con base de datos":`${members.length} miembros guardados en PostgreSQL`} action="Datos persistentes"/><div className="button-row end"><button className="primary" onClick={invite}>＋ Invitar miembro</button></div><div className="team-list">{members.map(member=><div key={member.id}><span className="avatar small">{initialsFromName(member.user.name)}</span><p><b>{member.user.name}</b><small>{member.user.email} · {member.user.status}</small></p><select value={member.role} onChange={event=>changeRole(member,event.target.value as TeamRole)}><option value="ORGANIZATION_ADMIN">{teamRoleLabel("ORGANIZATION_ADMIN")}</option><option value="SUPERVISOR">{teamRoleLabel("SUPERVISOR")}</option><option value="AGENT">{teamRoleLabel("AGENT")}</option></select><button onClick={()=>remove(member)}>Quitar</button></div>)}</div>{!loading&&!members.length&&<div className="empty"><b>Sin miembros</b><span>Invita al primer agente para probar persistencia real.</span></div>}</>
}
...
function BillingPanel({notify}:{notify:(s:string)=>void}) {
  ...
  const simulate=async(action:BillingMockAction,planPriceId?:string)=>{try{const result=await api.billingSimulate(action,planPriceId);setSubscription(result.subscription);notify(result.message)}catch(reason){notify(reason instanceof Error?reason.message:"No se pudo simular billing")}};
