...
import { api, type ApiConversation, type ApiDashboard, type ApiMessage, type BillingUsage, type KnowledgeKind, type KnowledgeRecord, type PlanPrice, type SubscriptionInfo } from "./api-client";
...
export default function NexoApp() {
  ...
  if (!loggedIn) return <Login onLogin={async(email,password) => { await api.login(email,password); setLoggedIn(true); }} />;
...
function Login({ onLogin }: { onLogin: (email:string,password:string) => Promise<void> }) {
  ...
  const submit = async(e: FormEvent) => { e.preventDefault(); setLoading(true);setError("");try{await onLogin(email,password)}catch(reason){setError(reason instanceof Error?reason.message:"No fue posible iniciar sesión");setLoading(false)} };
  return <div className="login-page"><div className="login-art"><div className="art-grid" /><div className="login-brand"><span className="brand-mark light">n</span>next.io <span>by Mercadia</span></div><div className="login-quote"><span>✦</span><h2>AI customer ops<br />para vender más rápido.</h2><p>Un inbox oscuro, rápido y conectado para responder, calificar y convertir conversaciones en revenue.</p><div className="channel-orbit"><b>IG</b><b>WA</b><b>FB</b><i /></div></div><small>Omnichannel AI stack for growth teams</small></div><div className="login-panel"><form onSubmit={submit} autoComplete="off"><span className="eyebrow">SECURE WORKSPACE</span><h1>Inicia sesión en next.io</h1><p>Accede a tu operación comercial y continúa tus conversaciones.</p><label>Correo electrónico<input type="email" name="login-email" autoComplete="off" value={email} onChange={event=>setEmail(event.target.value)} required /></label><label>Contraseña<div className="password"><input type="password" name="login-password" autoComplete="new-password" value={password} onChange={event=>setPassword(event.target.value)} required /><span>◉</span></div></label>{error&&<p role="alert" className="login-error">{error}</p>}<div className="form-row"><label className="check"><input type="checkbox" /> Recordarme</label><a>¿Olvidaste tu contraseña?</a></div><button className="primary login-submit" disabled={loading}>{loading ? "Ingresando…" : "Iniciar sesión  →"}</button><div className="demo-box"><span>●</span><div><b>Acceso conectado</b><p>La sesión se valida contra la API y la base de datos PostgreSQL.</p></div></div><small className="legal">Al continuar, aceptas los Términos y el Aviso de privacidad.</small></form></div></div>;
}
